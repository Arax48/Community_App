...
public class ListadosPagosController {
    private static final Logger logger = Logger.getLogger(ListadosPagosController.class.getName());
    ...
    private void cargarMascotas() {
        ...
        String query = "SELECT concepto, estado FROM pago";
        try {
            ResultSet resultSet = connection.getQueryTable(query);
            while (resultSet != null && resultSet.next()) {
                String concepto = resultSet.getString("concepto");
                String estado = resultSet.getString("estado");
                listViewMascotas.getItems().add(concepto + " - " + estado);
            }
        } catch (SQLException e) {
            logger.log(Level.SEVERE, "Error al cargar las pagos.", e);
        }
    }

    private void cargarMascotasPerdidas() {
        DbConnection connection = new DbConnection();
        String query = "SELECT concepto, estado FROM pago WHERE estado = 'P'";
        try {
            ResultSet resultSet = connection.getQueryTable(query);
            ...
            while (resultSet != null && resultSet.next()) {
                String concepto = resultSet.getString("concepto");
                String estado = resultSet.getString("estado");
                mascotasPerdidas.append(concepto).append(" - ").append(estado).append("\n");
            }
    ...
    private void mostrarDetalles(String nombreMascota) {
        DbConnection connection = new DbConnection();
        String query = "SELECT * FROM pago WHERE concepto = '" + nombreMascota + "'";
        try {
            ResultSet resultSet = connection.getQueryTable(query);
            ...
            logger.log(Level.SEVERE, "Error al cargar los detalles de las pagos.", e);
        }
    }
